...
class FasterWhisperPipeline(Pipeline):
    ...
    def _forward(self, model_inputs):
        ...
        # return {'text': outputs}
        model_outputs = self.model.generate_segment_batched(
    ...
    def transcribe_multilang(
        ...
        *,
        ...
        for seg in vad_segments:
            ...
            lang, prob = self.detect_segment_language(
                audio, seg["start"], seg["end"], SAMPLE_RATE, return_all=False
            )

            original_segment_lang_prob_length.append(
                {"lang": lang, "prob": prob, "len": seg_len}
            )

            if lang not in allowed_languages:
                if verbose:
                    print(
                        f"Language {lang} not in allowed languages, using fallback language {fallback_language}, probability: {prob: .3f}, segment length: {seg_len: .2f}s"
                    )
        ...
        for lang, segs in segments_by_lang.items():
            ...
            if self.suppress_numerals:
                opts = opts._replace(
                    suppress_tokens=list(
                        set(
                            opts.suppress_tokens + find_numeral_symbol_tokens(tokenizer)
                        )
                    )
                )
            ...
            for idx, out in enumerate(
                self.__call__(seg_iter(), batch_size=bs, num_workers=num_workers)
            ):
                if print_progress:
                ...
                avg_logprob = (
                    out["avg_logprob"][0] if bs in [0, 1, None] else out["avg_logprob"]
                )
                ...
                key = seg_out["start"]
                if (
                    key not in results_by_start
                    or avg_logprob > results_by_start[key][1]
                ):
        ...
        # ----------------------------- final merge -------------------------------
        final_segments = [
            v[0] for v in sorted(results_by_start.values(), key=lambda x: x[0]["start"])
        ]
    ...
    def detect_language(self, audio: np.ndarray):
        ...
        if audio.shape[0] < N_SAMPLES:
            print(
                "Warning: audio is shorter than 30s, language detection may be inaccurate. [language detection]"
            )
        ...
        for i in range(num_segments):
            segment = log_mel_spectrogram(
                audio[i * N_SAMPLES : (i + 1) * N_SAMPLES],
                n_mels=model_n_mels if model_n_mels is not None else 80,
                padding=(
                    0 if audio.shape[0] >= N_SAMPLES else N_SAMPLES - audio.shape[0]
                ),
            )
        ...
        if not language_of_segment:
            ...
            prob < self.language_probability_threshold
            for lang, prob in language_of_segment
        ):
            print(
        ...
        second_most_common_language = (
            sorted_languages[1] if len(sorted_languages) > 1 else (None, 0)
        )
