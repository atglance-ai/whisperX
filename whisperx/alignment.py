...
def load_align_model(language_code, device):
    ...
    try:
        ...
        raise ValueError(f"Error loading model from {model_name} from dir: {e}") from e
    pipeline_type = "huggingface"
    align_model = align_model.to(device)  # type: ignore
    align_dictionary = {
        char.lower(): code for char, code in processor.tokenizer.get_vocab().items()  # type: ignore
    }
